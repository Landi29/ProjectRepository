...
namespace CompareTexts
{
    public abstract class CompareText
    {
        ...
        private void SplitTempText() // Gets a list which contains each word in the given text 
        {
        ...
        public virtual List<string> FindDatabaseFilePaths(string tag, string trueOrFalse) // Gets all file paths in the database according to the parameters 
        {
            ...
            string path = findPath.PathToArticleDatabase; // Gets the path to the database 

            path = Path.Combine(path, tag, trueOrFalse); // Combines the toal path to the wanted directory
